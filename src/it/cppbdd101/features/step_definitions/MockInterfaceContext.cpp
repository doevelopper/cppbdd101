...
GIVEN("^a mock class with method foo$") 
{
   ...
   context->mock = std::make_shared<MockInterface>();
}

GIVEN("^the tests expects that play is called once$") 
{
   ScenarioScope<MockInterfaceContext> context{};
   EXPECT_CALL(*context->mock, play());
}

WHEN("^play is called on the mock$") 
{
   ScenarioScope<MockInterfaceContext> context{};
   context->mock->play();
}
